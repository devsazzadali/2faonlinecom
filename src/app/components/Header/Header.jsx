...
import { MdDarkMode, MdLightMode, MdMenu, MdClose, MdHistory, MdDeleteSweep, MdOutlineStickyNote2 } from "react-icons/md"; 
import Link from "next/link";

const Header = ({ onHistoryClick, historyCount, isDarkMode, onThemeToggle, onClearHistory }) => {
    ...
    <header className="sticky top-0 z-[1000] w-full bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-gray-100 dark:border-zinc-800 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Theme Toggle Button */}
          <button 
            className="p-2.5 rounded-xl border border-gray-200 dark:border-zinc-800 text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-900 transition-all" 
        ...
        {/* Mobile Dropdown (Without Framer Motion) */}
        {isMenuOpen && (
          <div className="absolute top-full right-5 w-64 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl shadow-2xl p-4 mt-2 flex flex-col gap-2 md:hidden">
            <Link 
              href="/note" 
              onClick={toggleMenu}
